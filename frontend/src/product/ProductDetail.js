  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const productData = await Lipsticks.getProduct(id).then((res) => 
        console.log(res)).catch((e) => console.log("error", e));
        //console.log("Product Data from React Component:", productData);
        setProduct(productData);

        // Fetch product comments after product information is retrieved
        const commentsData = await Lipsticks.getProductComment(id);
        console.log("Product Comments:", commentsData);
...
console.log(`product`,product);
  ...
  return (
    <Container fluid className='px-5 pt-5'>
      <Row>
        <Col md={6}>
          <Image src={product.imgUrl} alt={product.name} className='fade-in' fluid />
        </Col>
                ...
                <Form onSubmit={handleSubmit}>
                  <Form.Group controlId='comment'>
                    ...
                    <Form.Control
                      as='textarea'
                      row='3'
                      onChange={(e) => setComment(data => ({ ...data, comment: e.target.value }))}
